...
def embs_then_tokens(seq1,seq2,token_num_per_iter):
    result = torch.zeros_like(torch.cat((seq1, seq2), dim=1))
    out_pointer = 0
    in_pointer = 0
    for i in token_num_per_iter:
        result[:,out_pointer:out_pointer+i] = seq1[:,in_pointer:in_pointer+i]
        out_pointer +=i
        result[:,out_pointer:out_pointer+i] = seq2[:,in_pointer:in_pointer+i]
        out_pointer +=i
        in_pointer += i
...
def calculate_num_query_tokens_for_parallel_decoding_my(total_step, block_size):
    results=[]
    pre_idx=0
    for i in range(total_step):
        # how many tokens will be decoded at this step in total
        num_target_decoded_tokens = (
            1.0 - math.cos(math.pi / 2.0 * (i + 1) / total_step)
        ) * block_size + 1
        num_target_decoded_tokens = min(
            int(num_target_decoded_tokens), block_size
        )
        this_iter = max(1, num_target_decoded_tokens - pre_idx)
        results.append(this_iter)
        pre_idx+=this_iter

    assert sum(results)==block_size
