...
# from models.video_lavit_motion_tokenizer import build_motion_tokenizer,MotionVectorProcessor,extract_motions
from diffusers.utils import export_to_video, export_to_gif
from typing import Optional, Union, List

from dataset.video import video_transforms
...
class UCF101(torch.utils.data.Dataset):
    ...
    def __init__(self,
        ...
        self.video_lists = get_filelist(data_path)
        self.transform = transform
        self.temporal_sample = temporal_sample
        self.target_video_len = nframes
    ...
    #     labels=[]
    #     for i in index:
    #         video,class_index=self.__getitem__int(i)
    #         imgs.append(video.unsqueeze(0))
    #         labels.append(class_index.unsqueeze(0))
    #     return torch.cat(imgs,dim=0),torch.cat(labels,dim=0)

    def __getitem__(self, index):
        path = self.video_lists[index]
        class_name = path.split('/')[-2]
        class_index = self.class_to_idx[class_name]
        # each instance as a class
        # class_index=index

        vframes, aframes, info = torchvision.io.read_video(filename=path, pts_unit='sec', output_format='TCHW')
        # vframes=[]
        # for i in rang(50):
        #     vframes.append(torch.randn((3,256,256)))
...
def build_ucf(args):
    frame_interval = 1
    temporal_sample = video_transforms.TemporalRandomCrop(args.nframes * frame_interval)
    transform_ucf101 = transforms.Compose([
        video_transforms.ToTensorVideo(),  # TCHW
        video_transforms.RandomHorizontalFlipVideo(),
        video_transforms.UCFCenterCropVideo(args.img_size),
        normalize_01_into_pm1,
        # for spynet
        # transforms.Normalize(mean=[0.485, 0.456, 0.406], std=[0.229, 0.224, 0.225])
    ])
    dataset_train = UCF101(data_path=args.ucf_data_path, nframes=args.nframes, transform=transform_ucf101, temporal_sample=temporal_sample,return_caption=True)
    print(f'[Dataset]  {len(dataset_train.video_lists)=} videos.')
...
if __name__ == '__main__':
    ...
    parser.add_argument("--data-path", type=str, default="/mnt_alipayshnas/workspace2/binshao.ybb/TestDatasets/ucf101/UCF-101")
    ...
    ffs_dataset = UCF101(config.data_path,config.nframes, transform=transform_ucf101, temporal_sample=temporal_sample)
